...
// Print the content of the machine_code for debugging purpose
...
int main() {
    ...
    std::vector<uint8_t> machine_code {
        #ifdef __linux__
        0x48, 0xc7, 0xc0, 0x01, 0x00, 0x00, 0x00,           //Store the "write" system call number
        #elif __APPLE__
        0x48, 0xc7, 0xc0, 0x04, 0x00, 0x00, 0x02,           //Store the "write"
        #endif
        0x48, 0xc7, 0xc7, 0x01, 0x00, 0x00, 0x00,           //Store
        0x48, 0x8d, 0x35, 0x0a, 0x00, 0x00, 0x00,
        //Store the location of the string to write (3
        0x48, 0xc7, 0xc2, 0x00, 0x00, 0x00, 0x00,
        //Store the length of the string (initially
        0x0f, 0x05,
    ...
    // Get the memory required size for mmap
    size_t required_memory_size = estimate_memory_size(machine_code.size());

    uint8_t *mem = (uint8_t*) mmap(NULL, required_memory_size, PROT_READ | PROT_WRITE | PROT_EXEC, MAP_PRIVATE |
    MAP_ANONYMOUS, -1, 0);
    if (mem == MAP_FAILED)
    {
    ...
    void (*func)();
    // Cast the address of out generated code to a function pointer and call the function
    func = (void (*)()) mem;
    func();
